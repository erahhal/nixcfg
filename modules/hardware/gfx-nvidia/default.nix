...
let
  cfg = config.nixcfg.hardware.gfx-nvidia;
in
{
  key = "nixcfg/hardware/gfx-nvidia";

  options.nixcfg.hardware.gfx-nvidia.enable = lib.mkEnableOption "NVIDIA GPU support";
  ...
  in {
    ...
    hardware.nvidia = {
      ...
      # Use nvidia-open kernel modules for better kernel integration
      # May help with USB4 DP tunnel resume issues
      # mkDefault so nixos-hardware can override for older GPUs (e.g., Pascal)
