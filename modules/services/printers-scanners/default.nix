{ broken, config, lib, pkgs, ... }:
let
  cfg = config.nixcfg.services.printers-scanners;
in
{
  key = "nixcfg/services/printers-scanners";

  options.nixcfg.services.printers-scanners.enable = lib.mkEnableOption "Printer and scanner support";
